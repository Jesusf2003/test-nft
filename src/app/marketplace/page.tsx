...
const Closet: React.FC = () => {
    ...
    const fetchNFTs = async () => {
        ...
        try {
            ...
            const filteredInput = detailedListings.filter(nft => {
                if (filterType === "name" && searchValue) {
                    return nft.name.toLowerCase().includes(searchValue.toLowerCase());
                } else if (filterType === "seller" && searchValue) {
                    return nft.seller.toLowerCase().includes(searchValue.toLowerCase());
                } else {
                    return true;
                }
            })
            console.log('detailed listings: '+ filteredInput)
            setAssets(filteredInput);
        } catch (errr) {
            console.log(errr);
        } finally {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 pt-20 bg-white dark:bg-black min-h-screen">
            ...
            <div>
                ...
                <input type="text" placeholder="Buscar..." value={searchValue} onInput={handleInputChange} />
            </div>
